#ifndef _ROS_field_obj_TrackDetection_h
#define _ROS_field_obj_TrackDetection_h
...
#include "field_obj/TrackObject.h"

namespace field_obj
{

  class TrackDetection : public ros::Msg
  {
    public:
      typedef std_msgs::Header _header_type;
      _header_type header;
      uint32_t objects_length;
      typedef field_obj::TrackObject _objects_type;
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(objects_lengthT > objects_length)
        this->objects = (field_obj::TrackObject*)realloc(this->objects, objects_lengthT * sizeof(field_obj::TrackObject));
      objects_length = objects_lengthT;
      for( uint32_t i = 0; i < objects_length; i++){
      offset += this->st_objects.deserialize(inbuffer + offset);
        memcpy( &(this->objects[i]), &(this->st_objects), sizeof(field_obj::TrackObject));
      }
     return offset;
    }

    virtual const char * getType() override { return "field_obj/TrackDetection"; };
    virtual const char * getMD5() override { return "35ef6e490c4020b3af7c2b97386b528c"; };
